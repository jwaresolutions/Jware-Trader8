...
class BacktestEngine {
    ...
    async runBacktest(strategy, historicalData, config) {
        ...
        try {
            ...
            const portfolio = new portfolio_1.Portfolio(config.portfolio);
            const trades = [];
            const equityCurve = [];
                ...
                try {
                    ...
                    if (strategy && typeof strategy.executeStrategy === 'function') {
                        ...
                        for (const signal of signals) {
                            if (signal.type === 'BUY') {
                                const quantity = this.calculatePositionSize(signal.price, portfolio.getTotalValue(currentPrices), config.portfolio);
                                if (portfolio.canBuy(signal.symbol, signal.price, quantity)) {
                                    const trade = portfolio.openPosition(signal.symbol, signal.price, quantity, dataPoint.timestamp);
                                ...
                                if (portfolio.hasPosition(signal.symbol)) {
                                    const trade = portfolio.closePosition(signal.symbol, signal.price, dataPoint.timestamp);
            ...
            const summary = this.calculatePerformanceMetrics(trades.filter(t => t.status === 'CLOSED'), equityCurve, config.portfolio.initialCash);
     ...
     */
    calculatePerformanceMetrics(trades, equityCurve, initialValue) {
        ...
        const sharpeRatio = returns.length > 1 ? (0, utils_1.calculateSharpeRatio)(returns) : 0;
        // Calculate drawdown
        const equityValues = equityCurve.map(point => point.totalValue);
        const maxDrawdown = (0, utils_1.calculateMaxDrawdown)(equityValues);
        ...
        const winRate = (0, utils_1.calculateWinRate)(pnlValues);
        const profitFactor = (0, utils_1.calculateProfitFactor)(pnlValues);
     ...
     */
    calculatePositionSize(price, portfolioValue, portfolioConfig) {
